...
function AddFriend({handleAddfriend}) {
    ...
    const {setRenderHome, renderHome} = useContext(DataContext)
    ...
    const requestID = async ()=>{
        ...
        try {
            ...
            if(!res.data.user)
            {   
                return setErr("no user found")
            }

            else{
                
                setRenderHome(prev => !renderHome);
  ...
  return (
    <div className='bg-primary-bg absolute left-[260px] top-[55px] rounded-xl p-5 border border-gray-500 '>
        <CancelIcon onClick={handleAddfriend} className='text-[#f04747] cursor-pointer absolute -left-[8px] -top-[8px]'/>
    ...
    { err &&  <p className='pl-5 text-red-400' >{err}</p>}
    </div>
  )
}
